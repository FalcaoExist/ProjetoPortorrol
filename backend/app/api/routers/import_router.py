...
from app.api.schemas import StockImportPayload

router = APIRouter()

@router.post("/stock/import")
async def import_stock(payload: StockImportPayload, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user)):
    user_id = current_user.get("user_id")
    
    if not payload.filename.lower().endswith((".xlsx", ".xls")):
        raise HTTPException(400, "Envie um arquivo Excel (.xlsx)")
    
    if not payload.data:
        raise HTTPException(400, "O arquivo enviado está vazio ou não possui dados válidos.")
        
    background_tasks.add_task(process_background, payload.data, payload.filename, user_id)
