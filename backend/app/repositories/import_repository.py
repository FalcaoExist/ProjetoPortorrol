...
class ImportRepository:
    ...
    def save_sku(self, row):
        try:
            ...
            if marca_limpa and sku_id:
                ...
                sup_check = supabase.table("suppliers")\
                    .select("supplier_id")\
                    .ilike("name", marca_limpa)\
                    .execute()

                if sup_check.data:
                    supplier_id = sup_check.data[0]['supplier_id']
                else:
                    sup_insert = supabase.table("suppliers")\
                        .insert({"name": marca_limpa})\
                        .execute()
                ...
                if supplier_id:
                    ps_check = supabase.table("product_suppliers")\
                        .select("id")\
                        .eq("sku_id", sku_id)\
            ...
            logger.exception("Erro ao salvar SKU - codigo: %s", row.get('REFERENCIA'))
            raise
    ...
    def save_analysis(self, row, sku_id):
        try:
            ...
            ana_check = supabase.table("tb_analise_compra")\
                .select("id")\
                .eq("sku_id", sku_id)\
                .execute()

            if ana_check.data:
                supabase.table("tb_analise_compra")\
            ...
            logger.exception("Erro ao salvar análise - sku_id: %s", sku_id)
            raise
    ...
    def save_history(self, row, sku_id):
        try:
            ...
            logger.exception("Erro ao salvar histórico de vendas - sku_id: %s", sku_id)
            raise