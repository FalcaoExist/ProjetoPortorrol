...
class DashboardRepository:
    def __init__(self):
        self.view_analise = "vw_analise_reposicao"
        self.table_vendas = "tb_historico_vendas"
        self.table_config = "tb_configuracoes"

    def get_filtered_skus(self, status=None, branch=None, supplier=None, search=None, limit=1000000):
        try:
            query = supabase.table(self.view_analise).select(
            ...
            logger.exception(f"Erro ao buscar SKUs filtrados em {self.view_analise}")
            return []
    ...
    def get_history_by_sku(self, sku_id: int):
        return supabase.table(self.table_vendas).select("periodo_sequencia, quantidade, valor")\
            .eq("sku_id", sku_id).order("periodo_sequencia").limit(24).execute().data or []

    def get_aggregate_history(self):
        return supabase.table(self.table_vendas).select("periodo_sequencia, quantidade").limit(50000).execute().data or []
    ...
    def get_configuration(self, key: str):
        res = supabase.table(self.table_config).select("chave, valor").eq("chave", key).execute()
        return res.data[0] if res.data else None

    def update_configuration(self, key: str, value: Any):
        return supabase.table(self.table_config).update({"valor": str(value)}).eq("chave", key).execute()

    def get_supplier_budget(self, supplier_name: str):
        res = supabase.table("suppliers").select("budget, start, end")\
            .ilike("name", f"%{supplier_name}%").eq("is_active", True).execute()
        return res.data[0] if res.data else None
    ...
    def get_critical_skus(self, limit: int, supplier: str = None):
        ...
        rank_col = "ranking_fornecedor" if supplier else "ranking_global"
        if supplier: query = query.eq("fornecedor", supplier)
        return query.lte(rank_col, limit).order(rank_col).execute().data or []
